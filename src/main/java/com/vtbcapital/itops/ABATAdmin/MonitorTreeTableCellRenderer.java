...
public class MonitorTreeTableCellRenderer extends JTree implements
		TableCellRenderer {

	/** Die letzte Zeile, die gerendert wurde. */
	protected int visibleRow;

	private MonitorTreeTable treeTable;

	public MonitorTreeTableCellRenderer(MonitorTreeTable treeTable,
			TreeModel model) {
		super(model);
		...
		for (int i = 0; i < getRowCount(); i++) {
			...
			ActiveJob aj = (ActiveJob) getPathForRow(i).getLastPathComponent();
			if (aj.getLevel() < 3) {
				expandRow(i);
			}
		}
